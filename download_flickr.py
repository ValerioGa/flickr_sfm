...
if everything goes well, the process will download ~2500 images in the folder ./outputs
...
flickr_folder = args2.output_folder
print(" ".join(sys.argv))
print(args2)
images_folder = f"{args2.output_folder}/train"
...
def download_images_metadata(process_num, processes_num, flickr_ids_sublist, dict__flickr_id__info):
    for i, flickr_id in enumerate(flickr_ids_sublist):
        try:
            ...
            date = re.findall(r'Uploaded on (.*)',  response)[0]
...
parallelize_function(args2.processes_num, download_images_metadata, flickr_ids, dict__flickr_id__info)
...
parallelize_function(args2.processes_num, download_images, list__flickr_id__info)
