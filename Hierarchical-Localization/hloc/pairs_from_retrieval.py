...
def pairs_from_score_matrix(scores: torch.Tensor,
    ...
    k = min(skip_topk+num_select, scores.size(1)) #no error if there are not enough images
    ...
    if skip_topk == 0:
        ...
        sorted_values, sorted_indices = torch.topk(scores, k=skip_topk+num_select, dim=1)
        if skip_topk+num_select-1 < sorted_values.size(1):
            selected_values = sorted_values[:, list(range(skip_topk, skip_topk+num_select))]
            selected_indices = sorted_indices[:, list(range(skip_topk, skip_topk+num_select))]
        else:
            selected_values = sorted_values[:, list(range(skip_topk, sorted_values.size(1)))]
            selected_indices = sorted_indices[:, list(range(skip_topk, sorted_values.size(1)))]

        indices = selected_indices.cpu().numpy()
        valid = selected_values.isfinite().cpu().numpy()
...
def main(descriptors, output, num_matched,
    ...
    if query_list is not None:
        query_tens = np.isin(np.array(db_names)[None], query_list)
        self |= query_tens
    pairs = pairs_from_score_matrix(sim, self, num_matched, min_score, skip_topk)
